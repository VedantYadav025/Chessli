...
BitBoard validMoveGenerator::bitboardOfValidMoves(const Square& square) const {
  switch (auto [m_piece, m_color] = m_board.getPieceAtSquare(square); m_piece) {
	...
	case (Piece_type::Bishop):
	    return ((m_color == Color_type::White)
	     ? this->bishopValidBitBoard(square) & ~m_board.allWhitePiecesBitBoard()
	     : this->bishopValidBitBoard(square) & ~m_board.allBlackPiecesBitBoard());
    default:;
  }
}
...
BitBoard validMoveGenerator::kingValidBitBoard(const Square& square) {
...
BitBoard validMoveGenerator::knightValidBitBoard(const Square& square) {
...
BitBoard validMoveGenerator::whitePawnValidBitBoard(
   ...
   *        one more */
  const BitBoard white_pawn_two_steps = ((white_pawn_one_step & mask_rank[2]) << 8) &
                                  ~m_board.allPiecesBitBoard();

  /* the union of the movements dictate the possible moves forward
   *    available */
  const BitBoard white_pawn_valid_moves = white_pawn_one_step | white_pawn_two_steps;
  ...
  const BitBoard white_pawn_right_attack = (white_pawn_loc & clear_file[7]) << 9;
   ...
   *    attack/move. */
  const BitBoard WhitePawnValid = white_pawn_valid_moves | white_pawn_valid_attacks;
...
BitBoard validMoveGenerator::blackPawnValidBitBoard(
   ...
   *        one more */
  const BitBoard black_pawn_two_steps = ((black_pawn_one_step & mask_rank[5]) >> 8) &
                                  ~m_board.allPiecesBitBoard();

  /* the union of the movements dictate the possible moves forward
   *    available */
  const BitBoard black_pawn_valid_moves = black_pawn_one_step | black_pawn_two_steps;
  ...
  /* then check the right side of the pawn, minding the overflow File H */
  const BitBoard black_pawn_right_attack = (black_pawn_loc & clear_file[7]) >> 7;
  ...
  /* then we combine the two situations in which a white pawn can legally
   *    attack/move. */
  const BitBoard BlackPawnValid = black_pawn_valid_moves | black_pawn_valid_attacks;
...
BitBoard validMoveGenerator::bishopValidBitBoard(const Square& square) const {
    BitBoard occupancy = m_board.allPiecesBitBoard();
    unsetBit(occupancy, static_cast<int>(square));
